...
export async function renderPokemonCard(details) {
    ...
    const palette = await Vibrant.from(pokemonImg).getPalette();
    const backgroundColor = palette.LightMuted.hex;

    const pokemonCardTemplate = `
        <div class="pokemon__card" style="background-color: ${backgroundColor};">
    ...
    const pokemonContainer = document.querySelector(".pokemon__list");
